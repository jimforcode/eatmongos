...
public class DemoDistinct2 {
	public static void main(String[] args) throws UnknownHostException {

		MongoOperations template = new MongoTemplate(new Mongo(), "database");
		template.insert(new DistinctPojo("f1", "f2", "f3"));
		template.insert(new DistinctPojo("f11", "f2", "f3"));
		template.insert(new DistinctPojo("f111", "f2", "f3"));
		template.insert(new DistinctPojo("f1", "f2", "f3"));
		template.insert(new DistinctPojo("f2", null, "f3"));
		// 1. 加条件
		List<String> result = template.getCollection("distinctPojo").distinct("field1",
